...
void tlmPeripheral::netConnect(string localPort, tlmPeripheral &p, string remotePort) {
...
void tlmPeripheral::netConnect(string localPort, tlmProcessor &p, string remotePort) {
...
// with fixed decode
//
void tlmPeripheral::busConnect(tlmDecoder &target, string portName, Addr lo, Addr hi) {
...
//
void tlmPeripheral::busConnect(tlmDecoder &target, string portName) {

    tlmBusDynamicSlavePort  *s = new tlmBusDynamicSlavePort(parentModule, this, portName, 0);
    target.connect(*s);
}

void tlmPeripheral::busConnect(tlmDecoder &initiator, string portName, Uns32 addrBits) {
