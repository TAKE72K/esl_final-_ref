...
public:
    HANDLE_METHOD(optCommandP)
    GET_METHOD_PROTO(command,optCommandP)

   command(optCommandP existing);
...
class module : public object {
  ...
  public:
    ...
    Uns32       int32Value (std::string name, bool *set=0) { return opObjectParamInt32Value (h, name.c_str(), set); }
    Uns32       int64Value (std::string name, bool *set=0) { return opObjectParamInt64Value (h, name.c_str(), set); }
    Uns32       uns32Value (std::string name, bool *set=0) { return opObjectParamUns32Value (h, name.c_str(), set); }
    Uns32       uns64Value (std::string name, bool *set=0) { return opObjectParamUns64Value (h, name.c_str(), set); }
...
public:
    ...
    Uns64 clocksUntilTime(double endTime) {
        return opProcessorClocksUntilTimeDouble(handle(), endTime);
    }
    ...
    // Return how many more clocks this processor will execute until the given
    // time
    Uns64 clocksUntilTimeDouble(
        double          time
    ){
        return opProcessorClocksUntilTimeDouble(handle(),time);
    }
    ...
    // Return the address of the next complete instruction that this processor
    // will execute.
    Addr PCNext(
        Addr            pc
    ){
...
class parser {
  ...
  public:
    // construct simple parser without options and parse argc
