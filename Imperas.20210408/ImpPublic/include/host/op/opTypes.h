...
typedef enum optFlagTypeE {
    ...
    OP_FT_FC_ARGVPAIR   ///< The function called with each list of name=value
                        ///  pairs
} optFlagType;
...
typedef enum optLoaderControlsE {
    OP_LDR_DEFAULT          = 0x000, ///< No special features
    OP_LDR_PHYSICAL         = 0x001, ///< The loader will use physical as
                                     ///  opposed to logical addresses
    OP_LDR_VERBOSE          = 0x002, ///< The loader will report sections as
                                     ///  they are loaded
    OP_LDR_NO_ZERO_BSS      = 0x004, ///< The loader will not zero the BSS
                                     ///  section
    OP_LDR_SET_START        = 0x008, ///< The loader will set the PC of the
                                     ///  target processor (where known) to the
                                     ///  start address
    OP_LDR_SYMBOLS_ONLY     = 0x010, ///< The loader will read the symbols but
                                     ///  not load the program
    OP_LDR_ELF_USE_VMA      = 0x020, ///< The loader will load ELF files using
                                     ///  VMA addresses rather than LMA
    OP_LDR_FAIL_IS_ERROR    = 0x040, ///< Failure to load will prevent the
                                     ///  simulation from running
    OP_LDR_SIGN_EXTEND_ADDR = 0x080, ///< Sign-extend addresses from 32b to 64b
    OP_LDR_READ_FRAME_INFO  = 0x100, ///< Read DWARF frame information
    OP_LDR_LIMIT_32         = 0x200, ///< Limited to 32 bits per write (required
                                     ///  when loading via TLM)
    OP_LDR_LIMIT_64         = 0x400, ///< Limited to 64 bits per write (required
                                     ///  when loading via TLM)
    OP_LDR_SET_PROTECTION   = 0x800  ///< The loader will set memory protection
...
typedef enum optStrTypeE {
    ...
    OP_PROCROOT_EN            = 0x80000000, ///< Instance of a processor model
} optStrType;
...
/// @par
/// @a VA The processor virtual address (if different from the physical)
...
#define OP_VFS_EXTERNAL(_NAME)\
   extern void * _NAME ##_start; extern void * _NAME ##_end
...
/// @param _NAME () The label
#define OP_VFS_SYMBOLS(_NAME)\
   .lo = & _NAME ##_start, .hi = & _NAME ##_end
        

/// @def OP_VFS_SYMBOL
/// Use this to set the address of an embedded module
/// @param _NAME () The label
#define OP_VFS_SYMBOL(_NAME)\
   .lo = & _NAME
...
/// @param PSE     () Is a PSE
#define OP_VFS_VLNV(VENDOR,LIBRARY,NAME,VERSION,PSE)\
   .ref = { #VENDOR, #LIBRARY, #NAME, #VERSION, PSE }
...
/// @par  Simulator Parameter OP_FP_TRACEMEMVA.
/// @par  Type.
/// Bool
/// @par  Purpose.
/// Trace physical addresses (if false) or virtual addresses (if true)
/// @par  Can be applied to:
/// rootmodule@n

#define OP_FP_TRACEMEMVA     "tracememva"

/// @par  Simulator Parameter OP_FP_TRACEMEMADDR.
/// @par  Type.
/// Uns64
/// @par  Purpose.
/// Trace memory starting at this address
/// @par  Can be applied to:
/// rootmodule@n

#define OP_FP_TRACEMEMADDR   "tracememaddr"

/// @par  Simulator Parameter OP_FP_TRACEMEMSIZE.
/// @par  Type.
/// Uns64
/// @par  Purpose.
/// Trace memory this many bytes
/// @par  Can be applied to:
/// rootmodule@n

#define OP_FP_TRACEMEMSIZE   "tracememsize"
