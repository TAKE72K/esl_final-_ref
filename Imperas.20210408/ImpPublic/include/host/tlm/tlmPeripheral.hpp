...
public:
    ...
    void netConnect(string localPort, tlmPeripheral &p, string remotePort);
    void netConnect(string localPort, tlmProcessor  &p, string remotePort);
    void busConnect(tlmDecoder   &target, string portName, Addr lo, Addr hi);
    void busConnect(tlmDecoder   &target, string portName);
    void busConnect(tlmDecoder   &initiator, string portName, Uns32 addrBits);
};
