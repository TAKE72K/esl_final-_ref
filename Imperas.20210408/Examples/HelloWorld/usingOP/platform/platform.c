...
static OP_CONSTRUCT_FN(instantiateComponents) {
    ...
    if (strcmp(options.processortype, "riscv") == 0) {
        model       = opVLNVString(0, options.alternatevendor ? options.alternatevendor :
                                             "riscv.ovpworld.org", "processor",   "riscv",         "1.0", OP_EXTENSION, 0);
        semihosting = opVLNVString(0,        "riscv.ovpworld.org", "semihosting", "pk", "1.0", OP_EXTENSION, 0);

    } else if (strcmp(options.processortype, "mips32") == 0) {
        model       = opVLNVString(0, options.alternatevendor ? options.alternatevendor :
                                             "mips.ovpworld.org", "processor",   "mips32_r1r5",  "1.0", OP_EXTENSION, 0);
        semihosting = opVLNVString(0,        "mips.ovpworld.org", "semihosting", "mips32Newlib", "1.0", OP_EXTENSION, 0);

    } else if (strcmp(options.processortype, "arm") == 0) {
        model       = opVLNVString(0,options.alternatevendor ? options.alternatevendor :
                                             "arm.ovpworld.org", "processor",   "arm",       "1.0", OP_EXTENSION, 0);
        semihosting = opVLNVString(0,        "arm.ovpworld.org", "semihosting", "armNewlib", "1.0", OP_EXTENSION, 0);
