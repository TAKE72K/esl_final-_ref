...
//
// Monitor writes to the exchCount net
//
static OP_NET_WRITE_FN(exchCountMonitor) {
    opPrintf(">>> exchCount net written with %u\n", (Uns32)value);
}
...
int main(int argc, const char **argv) {
    ...
    const char *semihostFile = opVLNVString(
        0, "ovpworld.org", "modelSupport", "imperasExit", "1.0", OP_EXTENSION,
        True
    );
    ...
    // attach exchange instruction semihost library to processor
    const char *exchangeFile = "exchange/model."IMPERAS_SHRSUF;
    opProcessorExtensionNew(processor, exchangeFile, "exchange", 0);
    ...
    ////////////////////////////////////////////////////////////////////////////
    // ADD NET MONITOR OF EXCHANGE COUNTER
    ////////////////////////////////////////////////////////////////////////////

    // create exchCountNet net
    optNetP exchCountNet = opNetNew(mr, "exchCountNet", 0, 0);

    // connect exchCountNet net to the processor
    opObjectNetConnect(processor, exchCountNet, "exchCountNet");

    // install callback monitoring net
    opNetWriteMonitorAdd(exchCountNet, exchCountMonitor, 0);
