...
int main(int argc, const char **argv) {
    ...
    optNetP intr1Net = opNetNew(mr, "intr1Net", 0, 0);
    optNetP resetNet = opNetNew(mr, "resetNet", 0, 0);
    opObjectNetConnect(processor, intr0Net, "intr0");
    opObjectNetConnect(processor, intr1Net, "intr1");
    ...
    opNetWrite(resetNet, 1);
    simulate(processor, 5);
    opNetWrite(resetNet, 0);

    // run processor for 9 instructions
    simulate(processor, 9);

    // raise intr0 for one instruction
    opNetWrite(intr0Net, 1);
    simulate(processor, 1);
    opNetWrite(intr0Net, 0);

    // run processor for 9 instructions
    simulate(processor, 9);

    // raise intr1 for one instruction
    opNetWrite(intr1Net, 1);
    simulate(processor, 1);
    opNetWrite(intr1Net, 0);
