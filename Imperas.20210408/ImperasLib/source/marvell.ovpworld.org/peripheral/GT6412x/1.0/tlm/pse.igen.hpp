...
class GT6412x : public tlmPeripheral
{
  private:
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "marvell.ovpworld.org", "peripheral", "GT6412x", "1.0", OP_PERIPHERAL, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model marvell.ovpworld.org/peripheral/GT6412x/1.0");
        }
        return vlnv;
    }
