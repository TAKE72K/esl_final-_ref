...
class arc_0x32 : public tlmProcessor
{
  private:
    ...
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "arc.ovpworld.org", "processor", "arc", "1.0", OP_PROCESSOR, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model arc.ovpworld.org/processor/arc/1.0");
        }
        return vlnv;
    }
  ...
  public:
    ...
    arc_0x32(tlmModule &parent, sc_module_name name)
    ...
    arc_0x32(tlmModule &parent, sc_module_name name, params p, Uns32 INSTRUCTIONWidth=32, Uns32 DATAWidth=32)
