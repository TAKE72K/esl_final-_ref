...
class NCEPLMT100 : public tlmPeripheral
{
  private:
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "andes.ovpworld.org", "peripheral", "NCEPLMT100", "1.0", OP_PERIPHERAL, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model andes.ovpworld.org/peripheral/NCEPLMT100/1.0");
        }
        return vlnv;
    }
