...
class riscv_NX25F : public tlmProcessor
{
  private:
    ...
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "andes.ovpworld.org", "processor", "riscv", "1.0", OP_PROCESSOR, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model andes.ovpworld.org/processor/riscv/1.0");
        }
        return vlnv;
    }
  ...
  public:
    ...
    riscv_NX25F(tlmModule &parent, sc_module_name name)
    ...
    riscv_NX25F(tlmModule &parent, sc_module_name name, params p, Uns32 INSTRUCTIONWidth=56, Uns32 DATAWidth=56)
