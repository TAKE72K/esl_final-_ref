...
//
static Bool retiredCode(riscvP riscv) {
...
static RISCV_CSR_WRITEFN(mmisc_ctlW) {
    ...
    // update custom external interrupt mode (Machine mode only)
    riscv->MIMode = RD_XCSR_FIELD(object, mmisc_ctl, VEC_PLIC);
...
static void setPMPPriv(
    ...
    setDomainPriv(riscv, &riscv->pmpDomains, low, high, priv, "PMP", verbose);
}
...
//
inline static Bool getLMBounds(
    Bool   EN,
    Uns32  ILMSZ,
    Uns64  BPA,
    Uns64 *lowPAP,
...
//
static Bool getILMBounds(vmiosObjectP object, Uns64 *lowPAP, Uns64 *highPAP) {
    return getLMBounds(
        RD_XCSR_FIELD(object, milmb, EN),
        RD_XCSR_FIELD(object, micm_cfg, ILMSZ),
        RD_XCSR_FIELD(object, milmb, BPA),
        lowPAP,
        highPAP
    );
}
...
//
static Bool getDLMBounds(vmiosObjectP object, Uns64 *lowPAP, Uns64 *highPAP) {
    return getLMBounds(
        RD_XCSR_FIELD(object, mdlmb, EN),
        RD_XCSR_FIELD(object, mdcm_cfg, DLMSZ),
        RD_XCSR_FIELD(object, mdlmb, BPA),
        lowPAP,
        highPAP
    );
}
...
//
static Bool overlap(Uns64 low1, Uns64 high1, Uns64 low2, Uns64 high2) {
...
RISCV_PMA_CHECK_FN(andesPMACheck) {
    ...
    if(RD_XCSR_FIELD(object, mmsc_cfg, DPMA)) {
        ...
        // allow unrestricted access to ILM for any access
        if(
            getILMBounds(object, &lowLM, &highLM) &&
            overlap(lowLM, highLM, lowPA, highPA)
        ) {
            setPMAPriv(riscv, lowLM, highLM, MEM_PRIV_RWX, False);
            setPMPPriv(riscv, lowLM, highLM, MEM_PRIV_RWX, False);
        }

        // allow unrestricted access to DLM for data access
        if(
            (requiredPriv&MEM_PRIV_RW) &&
            getDLMBounds(object, &lowLM, &highLM) &&
            overlap(lowLM, highLM, lowPA, highPA)
        ) {
            setPMAPriv(riscv, lowLM, highLM, MEM_PRIV_RW, False);
            setPMPPriv(riscv, lowLM, highLM, MEM_PRIV_RW, False);
        }
    }
}
...
static const extCSRAttrs csrs[XCSR_ID(LAST)] = {
    ...
    //                 name                num    arch         extension       attrs     description                                      rCB              rwCB wCB
    XCSR_ATTR_TV_     (milmb,              0x7C0, 0,           ANV_ILMB,       0,0,0,0,  "Instruction Local memory Base (register only)", 0,               0,   0              ),
    XCSR_ATTR_TV_     (mdlmb,              0x7C1, 0,           ANV_DLMB,       0,0,0,0,  "Data Local memory Base (register only)",        0,               0,   0              ),
...
#define EXTENSION_CHECK_ID(_OBJ, _REQUIRED, _REG1, _REG2, _EXT) \
    if(                                                                     \
        (_REQUIRED & ANV_##_EXT) &&                                         \
        !RD_XCSR_FIELD(object, _REG1, I##_EXT) &&                           \
        !RD_XCSR_FIELD(object, _REG2, D##_EXT)                              \
    ) {                                                                     \
        return False;                                                       \
    }
...
static Bool extensionPresent(vmiosObjectP object, andesVariant required) {
    ...
    EXTENSION_CHECK(object, required, micm_cfg, ILMB);

    // validate mdcm_cfg-controlled register presence
    EXTENSION_CHECK(object, required, mdcm_cfg, DLMB);
...
void andesCSRInit(vmiosObjectP object) {
    ...
    if(RD_XCSR_FIELD(object, micm_cfg, ISZ)) {
    ...
    if(RD_XCSR_FIELD(object, mdcm_cfg, DSZ)) {
