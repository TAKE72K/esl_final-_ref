...
static Uns32 getInstructionSize(riscvP riscv, riscvAddr thisPC) {
    return vmirtInstructionBytes((vmiProcessorP) riscv, thisPC);
}
...
const static opAttrs attrsArray32[] = {
    ...
    ATTR32_VFCVT     (  VFWCVT_S_BF16,    RVANY,  "vfwcvt.s.bf16", "|0000000|.....|00000|100|.....|1011011|"),
    ATTR32_VFCVT     (  VFNCVT_BF16_S,    RVANY,  "vfncvt.bf16.s", "|0000000|.....|00001|100|.....|1011011|"),
...
static void decode32(riscvP riscv, vmiosObjectP object, andesInstrInfoP info) {
    ...
    riscvAddr lowPC  = info->thisPC;
    riscvAddr highPC = info->thisPC + 2;
    ...
    info->instruction = fetch2(riscv, lowPC) | (fetch2(riscv, highPC) << 16);
...
static void decode16(riscvP riscv, andesInstrInfoP info) {
    ...
    // get full instruction pattern
    info->instruction = fetch2(riscv, info->thisPC);
...
void andesDecode(
    ...
    info->bytes  = getInstructionSize(riscv, thisPC);
    info->thisPC = thisPC;
