...
class riscv_CV32E40Pv2 : public tlmProcessor
{
  private:
    params paramsForProc(params p) {
        p.set("variant", "CV32E40Pv2");
        return p;
    }
    ...
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "openhwgroup.ovpworld.org", "processor", "riscv", "1.0", OP_PROCESSOR, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model openhwgroup.ovpworld.org/processor/riscv/1.0");
        }
        return vlnv;
    }
  ...
  public:
    ...
    riscv_CV32E40Pv2(tlmModule &parent, sc_module_name name, params p, Uns32 INSTRUCTIONWidth=34, Uns32 DATAWidth=34, Uns32 CSRWidth=16)
