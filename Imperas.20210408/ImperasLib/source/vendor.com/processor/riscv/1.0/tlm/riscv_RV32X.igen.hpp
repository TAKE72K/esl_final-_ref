...
class riscv_RV32X : public tlmProcessor
{
  private:
    ...
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "vendor.com", "processor", "riscv", "1.0", OP_PROCESSOR, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model vendor.com/processor/riscv/1.0");
        }
        return vlnv;
    }
  ...
  public:
    ...
    riscv_RV32X(tlmModule &parent, sc_module_name name)
    ...
    riscv_RV32X(tlmModule &parent, sc_module_name name, params p, Uns32 INSTRUCTIONWidth=34, Uns32 DATAWidth=34)
