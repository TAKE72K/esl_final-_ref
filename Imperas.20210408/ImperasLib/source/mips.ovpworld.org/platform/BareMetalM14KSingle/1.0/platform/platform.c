...
int main(int argc, const char **argv) {
    ...
    const char *mipsModel    = opVLNVString(NULL, vendorP, "processor", "mips32_r1r5", "1.0", OP_PROCESSOR, 0);
...
static const char *selectVendor() {
    ...
    if(options.vendor) {
        ...
        for(i=0;i<NUMBER_VENDORS;i++){
            const char *mipsModel    = opVLNVString(NULL, vendor[index], "processor", "mips32_r1r5", "1.0", OP_PROCESSOR, 0);
            const char *mipsSemihost = opVLNVString(NULL, vendor[index], "semihosting", "mips32Newlib", "1.0", OP_EXTENSION, 0);
