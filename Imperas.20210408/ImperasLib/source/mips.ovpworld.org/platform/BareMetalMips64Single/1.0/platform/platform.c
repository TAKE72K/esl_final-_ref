...
int main(int argc, const char **argv) {
    ...
    const char *mips64Model    = opVLNVString(NULL, "mips.ovpworld.org", "processor", "mips64", "1.0", OP_PROCESSOR, 0);
    ...
    paramsCpu = opParamEnumSet(paramsCpu, "variant", "5KEc");
    paramsCpu = opParamBoolSet(paramsCpu, OP_FP_DEFAULTSEMIHOST, True);

    optBusP bus = opBusNew(mr, "bus", 36, 0, 0);
    ...
    opMemoryNew(
            ...
            OP_CONNECTIONS(
                OP_BUS_CONNECTIONS(
                    OP_BUS_CONNECT(bus, "sp1", .slave=1, .addrLo=0x000000000ULL, .addrHi=0xfffffffffULL)
                )
            ),
            0
    );
...
static void addPlatformDocumentation() {
    ...
    optDocNodeP Root1_node = opDocSectionAdd(0, "Root");
    {
        ...
        opDocTextAdd(doc_12_node, "Bare Metal Platform for an MIPS64 Processor."
                    ...
                    "The platform can be passed any application compiled to an MIPS64 elf format as the first argument."
                    "./platform.OS.exe --program application.elf");
        optDocNodeP doc_22_node = opDocSectionAdd(Root1_node, "Limitations");
        opDocTextAdd(doc_22_node, "BareMetal platform for execution of MIPS MIPS64 binary files compiled with MIPS SDK CrossCompiler toolchain.");
