...
class mips32_MIPS32R6 : public tlmProcessor
{
  private:
    params paramsForProc(params p) {
        p.set("variant", "MIPS32R6");
        return p;
    }
    ...
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "mips.ovpworld.org", "processor", "mips32", "1.0", OP_PROCESSOR, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model mips.ovpworld.org/processor/mips32/1.0");
        }
        return vlnv;
    }
  ...
  public:
    ...
    mips32_MIPS32R6(tlmModule &parent, sc_module_name name)
    ...
    mips32_MIPS32R6(tlmModule &parent, sc_module_name name, params p, Uns32 ISPRAMWidth=32, Uns32 DSPRAMWidth=32, Uns32 INSTRUCTIONWidth=32, Uns32 DATAWidth=32)
