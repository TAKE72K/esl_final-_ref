...
class mips64_P6600 : public tlmProcessor
{
  private:
    ...
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "mips.ovpworld.org", "processor", "mips64", "1.0", OP_PROCESSOR, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model mips.ovpworld.org/processor/mips64/1.0");
        }
        return vlnv;
    }
  ...
  public:
    ...
    mips64_P6600(tlmModule &parent, sc_module_name name)
    ...
    mips64_P6600(tlmModule &parent, sc_module_name name, params p, Uns32 INSTRUCTIONWidth=32, Uns32 DATAWidth=32, Uns32 USPRAMWidth=32)
