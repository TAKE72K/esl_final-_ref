...
class _16450 : public tlmPeripheral
{
  private:
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "national.ovpworld.org", "peripheral", "16450", "1.0", OP_PERIPHERAL, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model national.ovpworld.org/peripheral/16450/1.0");
        }
        return vlnv;
    }
