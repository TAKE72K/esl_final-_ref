...
class GD5446 : public tlmPeripheral
{
  private:
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "cirrus.ovpworld.org", "peripheral", "GD5446", "1.0", OP_PERIPHERAL, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model cirrus.ovpworld.org/peripheral/GD5446/1.0");
        }
        return vlnv;
    }
