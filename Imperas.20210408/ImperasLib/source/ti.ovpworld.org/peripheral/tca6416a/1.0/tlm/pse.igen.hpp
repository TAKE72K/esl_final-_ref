...
class tca6416a : public tlmPeripheral
{
  private:
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "ti.ovpworld.org", "peripheral", "tca6416a", "1.0", OP_PERIPHERAL, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model ti.ovpworld.org/peripheral/tca6416a/1.0");
        }
        return vlnv;
    }
