...
//
inline static Uns32 fetch2(vmiProcessorP processor, Addr thisPC) {
    return vmicxtFetch2Byte(processor, thisPC);
}

//
// Check for an opcode of 0x00000000 which indicates end of test
// Note: Do not fetch 4 bytes unless the low order 2 bytes are 0 to avoid a fetch
...
static Bool isEndtestInstr(vmiProcessorP processor, Addr thisPC) {
    return (fetch2(processor, thisPC)   == 0x0000 &&
            fetch2(processor, thisPC+2) == 0x0000);
}
