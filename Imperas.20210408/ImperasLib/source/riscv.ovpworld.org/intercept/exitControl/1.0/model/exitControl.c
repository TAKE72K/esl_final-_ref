...
static vmiParameter formals[] = {
    ...
    VMI_BOOL_PARAM_SPEC(paramValues,   onWriteToHost, 0,                "if function write_to_host() is executed exit simulation"),
...
static VMIOS_MORPH_FN(morphTime) {
    ...
    if(object->stopOnOpcode) {
        // get lower 16-bits
        Uns32 instruction = vmicxtFetch2Byte(processor, thisPC);
        if ((instruction & 3) == 3) {
            // 32-bit instruction
            Uns32 instTop = vmicxtFetch2Byte(processor, thisPC+2);
            instruction   = instruction | (instTop << 16);
        }
        if(instruction == object->opcode) {
