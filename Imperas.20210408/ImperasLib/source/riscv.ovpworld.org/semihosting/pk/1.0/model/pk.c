...
static void getXlen(
    ...
    //
    UnsArch misa;
    vmiRegInfoCP misaCP = vmiosGetRegDesc(processor, "misa");
    vmiosRegRead(processor, misaCP, &misa);

    object->xlen = 0;
    Uns8 mxl = (misa >> 30) & 0x3;
    if (mxl == 0) {
    ...
    if (mxl == 1) {
        ...
        vmiMessage("E", CPU_PREFIX, "MISA register error = " FMT_640Nx , misa); // LCOV_EXCL_LINE
        vmirtFinish(-1);
    }
}
...
VMIOS_CONSTRUCTOR_FN(constructor)
{
    ...
    object->args[3] = vmiosGetRegDesc(processor, "a3");
    object->args[4] = vmiosGetRegDesc(processor, "a4");
    object->args[5] = vmiosGetRegDesc(processor, "a5");
    // Default integer calling convention
    object->args[6] = vmiosGetRegDesc(processor, "a6");
    object->args[7] = vmiosGetRegDesc(processor, "a7");
    object->numArgs = REG_ARG_NUM;

    // Check processor for E Extension
    Bool extensionEPresent   = False;
    vmiRegInfoCP isA7Present = vmiosGetRegDesc(processor, "a7");
    vmiRegInfoCP misa        = vmiosGetRegDesc(processor, "misa");
    if(misa) {
        UnsArch misaValue;
        UnsArch mask = (1 << ('E' - 'A'));
    ...
    if(isA7Present) {
    	// expected
    }else if (extensionEPresent) {
        // ilp32e calling convention
        object->numArgs = 7;
        object->args[6] = vmiosGetRegDesc(processor, "t0");
    } else {
        vmiMessage("W", CPU_PREFIX "_UC", "Unexpected Configuration. "
                                          "No Register A7 and also no 'E' extension detected.");
    }
