...
class PLIC : public tlmPeripheral
{
  private:
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "riscv.ovpworld.org", "peripheral", "PLIC", "1.0", OP_PERIPHERAL, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model riscv.ovpworld.org/peripheral/PLIC/1.0");
        }
        return vlnv;
    }
  ...
  public:
    ...
    PLIC(tlmModule &parent, sc_module_name name)
        ...
        , port0(parent, this, "port0", 0x4000000) // static
