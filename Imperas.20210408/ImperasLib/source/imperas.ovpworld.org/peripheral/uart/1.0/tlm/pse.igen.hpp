...
class uart : public tlmPeripheral
{
  private:
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "imperas.ovpworld.org", "peripheral", "uart", "1.0", OP_PERIPHERAL, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model imperas.ovpworld.org/peripheral/uart/1.0");
        }
        return vlnv;
    }
