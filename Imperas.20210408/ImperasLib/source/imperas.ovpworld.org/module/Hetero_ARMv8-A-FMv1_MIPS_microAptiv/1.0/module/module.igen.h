...
static OP_CONSTRUCT_FN(instantiateComponents) {
    ...
    opDocTextAdd(doc_31_node, "ARM DUI 0677C and MIPS Processor");
    ...
    // Module Instance BareMetalMIPS

    const char *BareMetalMIPS_path = opVLNVString(
        0, // use the default VLNV path
    ...
    opParamEnumOverride(mi, "BareMetalMIPS/cpu/variant", "microAptivC");
    opModuleNew(
        mi,       // parent module
        BareMetalMIPS_path,       // modelfile
        "BareMetalMIPS",   // name
        OP_CONNECTIONS(
            OP_BUS_CONNECTIONS(
                OP_BUS_CONNECT(sharedBus_b, "pBusP")
        ...
        OP_PARAMS(
             OP_PARAM_UNS32_SET("addressbits", 32)
        )
    );
    ...
    opMemoryNew(
        ...
        OP_CONNECTIONS(
            OP_BUS_CONNECTIONS(
                OP_BUS_CONNECT(sharedBus_b, "sp1", .slave=1, .addrLo=0x0ULL, .addrHi=0xffffULL)
            )
        ),
        0
    );
