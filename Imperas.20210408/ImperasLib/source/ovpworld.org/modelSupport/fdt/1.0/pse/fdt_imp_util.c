...
static void fdtShowHeader(const void *fdt) {
    ...
    bhmMessage("I", PREFIX"_FDT", "Header magic 0x%x", magic);
    bhmMessage("I", PREFIX"_FDT", "Header off_dt_struct 0x%x", off_dt_struct);
    bhmMessage("I", PREFIX"_FDT", "Header off_dt_strings 0x%x", off_dt_strings);
    bhmMessage("I", PREFIX"_FDT", "Header off_mem_rsvmap 0x%x", off_mem_rsvmap);
    bhmMessage("I", PREFIX"_FDT", "Header version 0x%x", version);
    bhmMessage("I", PREFIX"_FDT", "Header last_comp_version 0x%x", last_comp_version);
    bhmMessage("I", PREFIX"_FDT", "Header size_dt_strings 0x%x", size_dt_strings);
    bhmMessage("I", PREFIX"_FDT", "Header size_dt_struct 0x%x", size_dt_struct);
}
        ...
        if (propLen) {
            ...
            if (isStringProp(prop, propLen)) {
                ...
                for (i = 0; i < propLen; i++) {
                    ...
                    bhmPrintf(" 0x%2x ('%c')", val, isprint(val) ? val : '.');
                }
            }
        }
