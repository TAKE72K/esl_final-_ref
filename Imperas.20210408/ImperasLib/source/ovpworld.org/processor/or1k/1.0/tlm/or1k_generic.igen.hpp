...
class or1k_generic : public tlmProcessor
{
  private:
    params paramsForProc(params p) {
        p.set("variant", "generic");
        return p;
    }
    ...
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "ovpworld.org", "processor", "or1k", "1.0", OP_PROCESSOR, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model ovpworld.org/processor/or1k/1.0");
        }
        return vlnv;
    }
  ...
  public:
    ...
    or1k_generic(tlmModule &parent, sc_module_name name)
    ...
    or1k_generic(tlmModule &parent, sc_module_name name, params p, Uns32 INSTRUCTIONWidth=32, Uns32 DATAWidth=32)
