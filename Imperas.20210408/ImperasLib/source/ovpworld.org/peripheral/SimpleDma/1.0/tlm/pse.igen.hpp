...
class SimpleDma : public tlmPeripheral
{
  private:
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "ovpworld.org", "peripheral", "SimpleDma", "1.0", OP_PERIPHERAL, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model ovpworld.org/peripheral/SimpleDma/1.0");
        }
        return vlnv;
    }
