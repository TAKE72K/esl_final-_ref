...
class zynq_7000_dmac : public tlmPeripheral
{
  private:
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "xilinx.ovpworld.org", "peripheral", "zynq_7000-dmac", "1.0", OP_PERIPHERAL, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model xilinx.ovpworld.org/peripheral/zynq_7000-dmac/1.0");
        }
        return vlnv;
    }
