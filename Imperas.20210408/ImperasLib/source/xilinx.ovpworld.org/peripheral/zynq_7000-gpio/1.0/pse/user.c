...
PPM_CONSTRUCTOR_CB(constructor) {
    ...
    char config[32];
    if (bhmStringParamValue("config", config, sizeof(config))) {
        if(PSE_DIAG_LOW)
            bhmMessage("I", PREFIX"_CFG", "Config '%s'", config);
        if (strcmp(config, "zc702") == 0) {
            // default initialisation
        } else if (strcmp(config, "zc706") == 0) {
            ...
            bhmMessage("E", PREFIX"_CFG", "Invalid config '%s'. Expected zc702 or zc706", config);
        }
    }
