...
class riscv_Ibex_RV32IMC : public tlmProcessor
{
  private:
    params paramsForProc(params p) {
        p.set("variant", "Ibex_RV32IMC");
        return p;
    }
    ...
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "lowrisc.ovpworld.org", "processor", "riscv", "1.0", OP_PROCESSOR, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model lowrisc.ovpworld.org/processor/riscv/1.0");
        }
        return vlnv;
    }
  ...
  public:
    ...
    riscv_Ibex_RV32IMC(tlmModule &parent, sc_module_name name)
    ...
    riscv_Ibex_RV32IMC(tlmModule &parent, sc_module_name name, params p, Uns32 INSTRUCTIONWidth=34, Uns32 DATAWidth=34)
