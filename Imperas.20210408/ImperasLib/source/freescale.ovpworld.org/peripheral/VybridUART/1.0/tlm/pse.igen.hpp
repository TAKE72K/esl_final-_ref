...
class VybridUART : public tlmPeripheral
{
  private:
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "freescale.ovpworld.org", "peripheral", "VybridUART", "1.0", OP_PERIPHERAL, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model freescale.ovpworld.org/peripheral/VybridUART/1.0");
        }
        return vlnv;
    }
  ...
  public:
    ...
    VybridUART(tlmModule &parent, sc_module_name name)
    ...
    VybridUART(tlmModule &parent, sc_module_name name, params p)
