...
class pwm : public tlmPeripheral
{
  private:
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "sifive.ovpworld.org", "peripheral", "pwm", "1.0", OP_PERIPHERAL, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model sifive.ovpworld.org/peripheral/pwm/1.0");
        }
        return vlnv;
    }
