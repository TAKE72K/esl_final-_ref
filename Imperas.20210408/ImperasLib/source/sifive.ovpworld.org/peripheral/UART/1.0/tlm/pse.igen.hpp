...
class UART : public tlmPeripheral
{
  private:
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "sifive.ovpworld.org", "peripheral", "UART", "1.0", OP_PERIPHERAL, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model sifive.ovpworld.org/peripheral/UART/1.0");
        }
        return vlnv;
    }
  ...
  public:
    ...
    UART(tlmModule &parent, sc_module_name name)
        ...
        , bport1(parent, this, "bport1", 0x1c) // static
