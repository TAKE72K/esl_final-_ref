...
VMIOS_DOC_FN(sifiveDoc) {
    ...
    sifiveConfigCP config      = sifiveExtConfig((riscvP)processor);
    ...
    if(config->variant & S5V_CSR) {
        ...
        if(config->variant & S5V_BPM) {
            docBPM(csrSection);
        }
        if(config->variant & S5V_FDCP) {
            docFDCP(csrSection);
        }
    }

    if(config->variant & S5V_INST) {
