...
class riscv_E76 : public tlmProcessor
{
  private:
    ...
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "sifive.ovpworld.org", "processor", "riscv", "1.0", OP_PROCESSOR, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model sifive.ovpworld.org/processor/riscv/1.0");
        }
        return vlnv;
    }
  ...
  public:
    ...
    riscv_E76(tlmModule &parent, sc_module_name name)
    ...
    riscv_E76(tlmModule &parent, sc_module_name name, params p, Uns32 INSTRUCTIONWidth=34, Uns32 DATAWidth=34)
