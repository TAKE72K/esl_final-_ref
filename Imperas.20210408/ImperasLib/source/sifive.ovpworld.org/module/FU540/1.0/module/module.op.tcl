...
iadddocumentation -name Limitations  -text "The Serial Peripheral Interface (SPI) is not modeled. Instead a Virtio Block MMIO device has been added at reserved address 0x1f000000, using interrupt 54."
iadddocumentation -name Limitations  -text "The General Purpose Input/Output Controller (GPIO) is not modeled."
iadddocumentation -name Limitations  -text "The One-Time Programmable Memory Interface (OTP) is not modeled."
iadddocumentation -name Limitations  -text "DDR controller is not modeled. DDR memory is modeled as RAM."
...
set mem_size  0x40000000
ihwaddmemory  -instancename mem1 -type ram
ihwconnect \
 -instancename mem1 -busslaveport sp1 -bus bus0 \
...
# PLIC
ihwaddperipheral -instancename plic -vendor riscv.ovpworld.org -library peripheral -type PLIC -version 1.0
...
ihwconnect -instancename plic -bus bus0 -busslaveport port0 -loaddress 0x0C000000 -hiaddress 0x0FFFFFFF
...
# Address to load boot image (Base of RAM):
ihwsetparameter  -handle       smartLoader  -name bootaddr   -value 0x80000000 -type address
# Address for smart loader to write boot code and DTB (Use mask ROM):
ihwsetparameter  -handle       smartLoader  -name slbootaddr -value 0x10000 -type address
