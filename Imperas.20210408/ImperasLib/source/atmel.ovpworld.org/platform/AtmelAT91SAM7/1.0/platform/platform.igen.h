...
static void cmdParserAddUserArgs(optCmdParserP parser) {
    opCmdParserAdd(parser, "kernel"  , 0 , 0, "user platform config", OP_FT_STRINGVAL     , &options.kernel, "The Linux Kernel image e.g. linux", OP_AC_ALL, 0, 0);
    opCmdParserAdd(parser, "uartport", 0 , 0, "user platform config", OP_FT_UNS64VAL      , &options.uartport, "Set the port number to open on the UART and wait for connection, by default a console will be opened.", OP_AC_ALL, 0, 0);
}
