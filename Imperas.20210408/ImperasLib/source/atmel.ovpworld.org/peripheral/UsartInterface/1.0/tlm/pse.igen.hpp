...
class UsartInterface : public tlmPeripheral
{
  private:
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "atmel.ovpworld.org", "peripheral", "UsartInterface", "1.0", OP_PERIPHERAL, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model atmel.ovpworld.org/peripheral/UsartInterface/1.0");
        }
        return vlnv;
    }
