...
class Uart : public tlmPeripheral
{
  private:
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "altera.ovpworld.org", "peripheral", "Uart", "1.0", OP_PERIPHERAL, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model altera.ovpworld.org/peripheral/Uart/1.0");
        }
        return vlnv;
    }
