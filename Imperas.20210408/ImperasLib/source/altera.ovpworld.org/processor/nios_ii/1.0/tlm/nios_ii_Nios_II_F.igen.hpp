...
class nios_ii_Nios_II_F : public tlmProcessor
{
  private:
    params paramsForProc(params p) {
        p.set("variant", "Nios_II_F");
        return p;
    }
    ...
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "altera.ovpworld.org", "processor", "nios_ii", "1.0", OP_PROCESSOR, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model altera.ovpworld.org/processor/nios_ii/1.0");
        }
        return vlnv;
    }
  ...
  public:
    ...
    nios_ii_Nios_II_F(tlmModule &parent, sc_module_name name)
    ...
    nios_ii_Nios_II_F(tlmModule &parent, sc_module_name name, params p, Uns32 INSTRUCTIONWidth=32, Uns32 DATAWidth=32)
