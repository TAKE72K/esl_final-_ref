...
static OP_CONSTRUCT_FN(instantiateComponents) {
    ...
    opDocTextAdd(doc_11_node, "This module implements a Processing Node for the SafePower Public Demonstrator in the Xilinx Zynq Programmable Logic (PL).\nThis PL configuration instances one Xilinx MicroBlaze processor with a local memory and a TTEL NoC interface peripheral.");
...
////////////////////////////////////////////////////////////////////////////////
    ...
    opMemoryNew(
        ...
        "debug_gpio",
        OP_PRIV_RWX,
        (0x4000ffffULL) - (0x40000000ULL),
        OP_CONNECTIONS(
            OP_BUS_CONNECTIONS(
                OP_BUS_CONNECT(pBus_b, "sp1", .slave=1, .addrLo=0x40000000ULL, .addrHi=0x4000ffffULL)
            )
        ),
        0
    );
    ...
    opMemoryNew(
        ...
        "axi_gpio",
        OP_PRIV_RWX,
        (0x4003ffffULL) - (0x40030000ULL),
        OP_CONNECTIONS(
            OP_BUS_CONNECTIONS(
                OP_BUS_CONNECT(pBus_b, "sp1", .slave=1, .addrLo=0x40030000ULL, .addrHi=0x4003ffffULL)
            )
        ),
        0
    );
...
optModuleAttr modelAttrs = {
    ...
    .vlnv          = {
        ...
        .library = "module",
        .name    = "Zynq_PL_TTELNoC_processing_node_public_demonstrator",
        .version = "1.0"
    },
