...
class NostrumNode : public tlmPeripheral
{
  private:
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "safepower.ovpworld.org", "peripheral", "NostrumNode", "1.0", OP_PERIPHERAL, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model safepower.ovpworld.org/peripheral/NostrumNode/1.0");
        }
        return vlnv;
    }
