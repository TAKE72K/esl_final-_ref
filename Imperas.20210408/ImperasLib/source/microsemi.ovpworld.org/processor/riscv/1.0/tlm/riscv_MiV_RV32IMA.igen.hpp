...
class riscv_MiV_RV32IMA : public tlmProcessor
{
  private:
    params paramsForProc(params p) {
        p.set("variant", "MiV_RV32IMA");
        return p;
    }
    ...
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "microsemi.ovpworld.org", "processor", "riscv", "1.0", OP_PROCESSOR, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model microsemi.ovpworld.org/processor/riscv/1.0");
        }
        return vlnv;
    }
  ...
  public:
    ...
    riscv_MiV_RV32IMA(tlmModule &parent, sc_module_name name)
    ...
    riscv_MiV_RV32IMA(tlmModule &parent, sc_module_name name, params p, Uns32 INSTRUCTIONWidth=34, Uns32 DATAWidth=34)
