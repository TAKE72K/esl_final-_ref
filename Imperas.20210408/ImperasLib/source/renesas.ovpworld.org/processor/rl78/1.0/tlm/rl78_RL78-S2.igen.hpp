...
class rl78_RL78_S2 : public tlmProcessor
{
  private:
    params paramsForProc(params p) {
        p.set("variant", "RL78-S2");
        return p;
    }
    ...
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "renesas.ovpworld.org", "processor", "rl78", "1.0", OP_PROCESSOR, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model renesas.ovpworld.org/processor/rl78/1.0");
        }
        return vlnv;
    }
  ...
  public:
    ...
    rl78_RL78_S2(tlmModule &parent, sc_module_name name)
    ...
    rl78_RL78_S2(tlmModule &parent, sc_module_name name, params p, Uns32 INSTRUCTIONWidth=20, Uns32 DATAWidth=20)
