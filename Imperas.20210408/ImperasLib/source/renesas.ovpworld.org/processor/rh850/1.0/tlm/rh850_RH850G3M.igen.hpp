...
class rh850_RH850G3M : public tlmProcessor
{
  private:
    params paramsForProc(params p) {
        p.set("variant", "RH850G3M");
        return p;
    }
    ...
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "renesas.ovpworld.org", "processor", "rh850", "1.0", OP_PROCESSOR, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model renesas.ovpworld.org/processor/rh850/1.0");
        }
        return vlnv;
    }
  ...
  public:
    ...
    rh850_RH850G3M(tlmModule &parent, sc_module_name name)
    ...
    rh850_RH850G3M(tlmModule &parent, sc_module_name name, params p, Uns32 INSTRUCTIONWidth=28, Uns32 DATAWidth=28)
