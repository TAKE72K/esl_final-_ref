...
class v850_V850E2R : public tlmProcessor
{
  private:
    params paramsForProc(params p) {
        p.set("variant", "V850E2R");
        return p;
    }
    ...
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "renesas.ovpworld.org", "processor", "v850", "1.0", OP_PROCESSOR, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model renesas.ovpworld.org/processor/v850/1.0");
        }
        return vlnv;
    }
  ...
  public:
    ...
    v850_V850E2R(tlmModule &parent, sc_module_name name)
    ...
    v850_V850E2R(tlmModule &parent, sc_module_name name, params p, Uns32 INSTRUCTIONWidth=28, Uns32 DATAWidth=28)
