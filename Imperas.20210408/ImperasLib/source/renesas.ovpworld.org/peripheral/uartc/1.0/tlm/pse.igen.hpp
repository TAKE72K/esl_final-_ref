...
class uartc : public tlmPeripheral
{
  private:
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "renesas.ovpworld.org", "peripheral", "uartc", "1.0", OP_PERIPHERAL, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model renesas.ovpworld.org/peripheral/uartc/1.0");
        }
        return vlnv;
    }
