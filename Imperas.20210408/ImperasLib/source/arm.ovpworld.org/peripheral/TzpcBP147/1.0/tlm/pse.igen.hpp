...
class TzpcBP147 : public tlmPeripheral
{
  private:
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "arm.ovpworld.org", "peripheral", "TzpcBP147", "1.0", OP_PERIPHERAL, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model arm.ovpworld.org/peripheral/TzpcBP147/1.0");
        }
        return vlnv;
    }
