...
class IntICP : public tlmPeripheral
{
  private:
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "arm.ovpworld.org", "peripheral", "IntICP", "1.01", OP_PERIPHERAL, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model arm.ovpworld.org/peripheral/IntICP/1.01");
        }
        return vlnv;
    }
