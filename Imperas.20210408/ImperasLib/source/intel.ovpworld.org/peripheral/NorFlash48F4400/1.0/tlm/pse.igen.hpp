...
class NorFlash48F4400 : public tlmPeripheral
{
  private:
    const char *getModel() {
        const char *vlnv = opVLNVString (NULL, "intel.ovpworld.org", "peripheral", "NorFlash48F4400", "1.0", OP_PERIPHERAL, 1);
        if(!vlnv) {
            opMessage("F", "TLM_FFM", "Failed to find model intel.ovpworld.org/peripheral/NorFlash48F4400/1.0");
        }
        return vlnv;
    }
